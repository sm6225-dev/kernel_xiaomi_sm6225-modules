...
#ifdef CONFIG_COMPAT
static int __maybe_unused msm_pcm_compat_ioctl(struct snd_soc_component *component, struct snd_pcm_substream *substream,
		unsigned int cmd, void __user *arg)
{
	/* we only handle RESET which is common for both modes */
	return msm_pcm_ioctl(component, substream, cmd, (void *)arg);
}
#endif
