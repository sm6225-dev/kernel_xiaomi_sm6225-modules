...
#ifndef SNDRV_HWDEP_IFACE_AUDIO_BE
#define SNDRV_HWDEP_IFACE_AUDIO_BE 100
#endif
...
static int msm_pcm_ioctl(struct snd_soc_component *component, struct snd_pcm_substream *substream,
...
#ifdef CONFIG_COMPAT
static int __maybe_unused msm_pcm_compat_ioctl(struct snd_pcm_substream *substream,
				unsigned int cmd, void *arg)
{
	/* we only handle RESET which is common for both modes */
	return msm_pcm_ioctl(NULL, substream, cmd, arg);
}
#endif
...
static int msm_pcm_copy(struct snd_soc_component *component, struct snd_pcm_substream *substream, int a,
...
static int __maybe_unused msm_pcm_set_volume(struct msm_audio *prtd, uint32_t volume)
{
	return 0;
}

static int __maybe_unused msm_pcm_volume_ctl_get(struct snd_kcontrol *kcontrol,
		      struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}

static int __maybe_unused msm_pcm_volume_ctl_put(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	return 0;
}
