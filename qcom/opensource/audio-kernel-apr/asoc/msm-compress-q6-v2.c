...
struct msm_compr_gapless_state {
	...
	union snd_codec_options_qc codec_options;
};
...
static int msm_compr_read_buffer(struct msm_compr_audio *prtd)
{
	...
	param.len = buffer_length;
	param.uid = buffer_length;
	param.flags = snd_codec_get_flags(&prtd->codec_param.codec);
					     ...
					     bool use_gapless_codec_options)
{
	...
	if (use_gapless_codec_options)
		codec_options = QC_CODEC_OPTIONS(
			&prtd->gapless_state.codec_options);
	else
		codec_options = QC_CODEC_OPTIONS(
			...
			(struct snd_compr_stream *cstream)
{
	...
	if (snd_codec_get_flags(&prtd->codec_param.codec) & COMPRESSED_TIMESTAMP_FLAG)
...
static int msm_compr_playback_open(struct snd_compr_stream *cstream)
{
	...
	if (pdata->is_in_use[rtd->dai_link->id] == true) {
		pr_err("%s: %s is already in use, err: %d\n",
			__func__, asoc_rtd_to_cpu(rtd, 0)->name, -EBUSY);
		return -EBUSY;
	}
				...
				struct snd_compr_params *params)
{
	...
	if (snd_codec_get_flags(&params->codec) & COMPRESSED_PERF_MODE_FLAG) {
...
static int __maybe_unused msm_compr_set_next_track_param(
				struct snd_compr_stream *cstream,
...
static int msm_compr_new(struct snd_soc_component *component,
			 struct snd_soc_pcm_runtime *rtd)
{
	int rc;
