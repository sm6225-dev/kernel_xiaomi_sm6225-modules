...
static int __maybe_unused msm_lsm_ioctl_compat(struct snd_pcm_substream *substream,
...
static int msm_lsm_ioctl(struct snd_soc_component *component,
			 struct snd_pcm_substream *substream,
			 unsigned int cmd, void __user *arg)
{
	int err = 0;
...
static snd_pcm_uframes_t msm_lsm_pcm_pointer(
	struct snd_soc_component *component, struct snd_pcm_substream *substream)
{
	struct snd_pcm_runtime *runtime = substream->runtime;
	struct lsm_priv *prtd = runtime->private_data;
	struct snd_soc_pcm_runtime *rtd;
...
static int msm_lsm_pcm_copy(struct snd_soc_component *component, struct snd_pcm_substream *substream, int ch,
...
static int __maybe_unused msm_lsm_app_type_cfg_ctl_put(struct snd_kcontrol *kcontrol,
...
static int __maybe_unused msm_lsm_app_type_cfg_ctl_get(struct snd_kcontrol *kcontrol,
					struct snd_ctl_elem_value *ucontrol)
{
	u64 fe_id = kcontrol->private_value;
	int session_type = SESSION_TYPE_TX;
...
static int __maybe_unused msm_lsm_afe_data_ctl_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	u64 fe_id = kcontrol->private_value;
...
static int __maybe_unused msm_lsm_afe_data_ctl_get(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	u64 fe_id = kcontrol->private_value;
	uint16_t afe_data_format = 0;
...
static int msm_asoc_lsm_new(struct snd_soc_component *component,
			    struct snd_soc_pcm_runtime *rtd)
{
