...
#include <uapi/sound/compress_params_compat.h>
...
static int msm_transcode_loopback_open(struct snd_soc_component *component, struct snd_compr_stream *cstream)
{
	int ret = 0;
...
static int msm_transcode_loopback_free(struct snd_soc_component *component, struct snd_compr_stream *cstream)
{
...
static int msm_transcode_loopback_trigger(struct snd_soc_component *component, struct snd_compr_stream *cstream, int cmd)
{
	struct snd_compr_runtime *runtime = cstream->runtime;
	struct msm_transcode_loopback *trans = runtime->private_data;
...
static int msm_transcode_loopback_set_params(struct snd_soc_component *component, struct snd_compr_stream *cstream,
					   struct snd_compr_params *params)
{
	...
	if (cstream->direction == SND_COMPRESS_PLAYBACK) {
		if (params->codec.id == SND_AUDIOCODEC_PCM) {
...
static int msm_transcode_loopback_get_caps(struct snd_soc_component *component, struct snd_compr_stream *cstream,
				struct snd_compr_caps *caps)
{
	...
	pr_debug("%s\n", __func__);
	if (cstream->direction == SND_COMPRESS_CAPTURE)
		memcpy(caps, &trans->source_compr_cap,
		       sizeof(struct snd_compr_caps));
	else
		memcpy(caps, &trans->sink_compr_cap,
		       sizeof(struct snd_compr_caps));
	return 0;
}
...
static int msm_transcode_loopback_set_metadata(struct snd_soc_component *component, struct snd_compr_stream *cstream,
...
static int msm_transcode_add_audio_effects_control(struct snd_soc_pcm_runtime *rtd)
{
	...
	pr_debug("%s: added new compr FE with name %s, id %d, cpu dai %s, device no %d\n", __func__,
						...
						asoc_rtd_to_cpu(rtd, 0)->name, rtd->pcm->device);
...
static int msm_transcode_loopback_new(struct snd_soc_component *component, struct snd_soc_pcm_runtime *rtd)
{
	int rc;
...
static struct snd_compress_ops msm_transcode_loopback_ops = {
