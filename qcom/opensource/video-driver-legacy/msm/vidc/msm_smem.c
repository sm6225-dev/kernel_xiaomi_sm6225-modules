...
#endif
#ifndef ION_FLAG_CACHED_NEEDS_SYNC
	...
	struct msm_smem *mem, u32 sid)
{
	...
	const char *heap_name = "qcom,system";
	...
	if (map_kernel) {
		...
		if (rc) {
			s_vpr_e(sid, "Failed to map shared mem in kernel: %d\n", rc);
			rc = -EIO;
			goto fail_map;
		}
...
static int free_dma_mem(struct msm_smem *mem, u32 sid)
{
	...
	if (mem->kvaddr) {
		dma_buf_vunmap(mem->dma_buf, &mem->dmabuf_map);
		mem->kvaddr = NULL;
