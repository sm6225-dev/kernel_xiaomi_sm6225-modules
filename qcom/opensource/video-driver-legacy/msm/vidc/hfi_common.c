...
#ifndef SMEM_IMAGE_VERSION_TABLE
#define SMEM_IMAGE_VERSION_TABLE 469
#endif

#ifndef stm_log_inv_ts
#define stm_log_inv_ts(entity_id, proto_id, data, size) (0)
#endif

#ifndef CLKFLAG_NORETAIN_PERIPH
#define CLKFLAG_NORETAIN_PERIPH 0x1
#endif
#ifndef CLKFLAG_RETAIN_PERIPH
#define CLKFLAG_RETAIN_PERIPH 0x2
#endif
static inline int clk_set_flags(struct clk *clk, unsigned long flags) { return 0; }
...
static int __protect_cp_mem(struct venus_hfi_device *device)
{
	...
	u32 cp_start = 0x0;
	u32 cp_size = 0x0;
	u32 cp_nonpixel_start = 0x0;
	u32 cp_nonpixel_size = 0x0;
...
static int __load_fw_to_memory(struct platform_device *pdev, const char *fw_name)
{
	int rc = 0;
	...
	if (rc) {
		d_vpr_e("%s: failed to read \"memory-region\" resource, error %d\n",
			__func__, rc);
		return rc;
	}
	phys = res.start;
	...
	scnprintf(firmware_name, sizeof(firmware_name), "%s.mbn", fw_name ? fw_name : "venus");
	rc = firmware_request_nowarn(&firmware, firmware_name, &pdev->dev);
	if (rc) {
		scnprintf(firmware_name, sizeof(firmware_name), "%s.mdt", fw_name ? fw_name : "venus");
		rc = firmware_request_nowarn(&firmware, firmware_name, &pdev->dev);
		if (rc) {
	...
	if (fw_size < 0 || res_size < (size_t)fw_size) {
		d_vpr_e("%s: out of bound fw image fw size: %zd, res_size: %zu\n",
	...
	if (!virt) {
		d_vpr_e("%s: failed to remap fw memory phys %pa\n", __func__, &phys);
		release_firmware(firmware);
		return -ENOMEM;
	}
...
static int __load_fw(struct venus_hfi_device *device)
{
	...
	if (!device->res->firmware_base) {
		if (!device->resources.fw.cookie) {
			int pas_id = __load_fw_to_memory(device->res->pdev, device->res->fw_name);
			if (pas_id <= 0) {
			...
			device->resources.fw.cookie = (void *)(uintptr_t)pas_id;
		}
	} else {
...
fail_protect_mem:
	if (device->resources.fw.cookie) {
		qcom_scm_pas_shutdown((uintptr_t)device->resources.fw.cookie);
		device->resources.fw.cookie = NULL;
	}
fail_load_fw:
	call_venus_op(device, power_off, device);
