		...
		struct context_bank_info *cb, struct device *dev)
{
	...
	dma_set_seg_boundary(dev, (unsigned long)~0ULL);
		...
		struct msm_vidc_core *core)
{
	...
	if (rc) {
		if (!strcmp(cb->name, "venus_sec_non_pixel"))
			cb->buffer_type = 0x480;
		else if (!strcmp(cb->name, "venus_sec_bitstream"))
			cb->buffer_type = 0x241;
		else if (!strcmp(cb->name, "venus_sec_pixel"))
			cb->buffer_type = 0x106;
		else
			cb->buffer_type = 0xfff;
		rc = 0;
	}
